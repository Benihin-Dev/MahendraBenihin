...
import TempletForReview from "./TempletForReview";

export default function SliderForReview({ data }) {
  ...
  return (
    <div className={` w-full mt-10 px-1 sm:px-0  h-full lg:pr-5 relative `}>
      <Slider
        ref={sliderRef}
        dots={false}
        infinite={true}
      ...
      >
        {slides.map((item, index) => (
          <div key={index} className=" w-full px-1 sm:px-4  h-full   ">
            <TempletForReview data={item} />
          </div>
        ))}
      </Slider>
